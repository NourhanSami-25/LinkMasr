								...
								<div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
									...
									<div class="page-title d-flex flex-column justify-content-center gap-1 me-3">
										...
										<h1
											...
											{{ __('general.all_requests_logs') }} </h1>
										<!--end::Title-->
										<!--begin::Breadcrumb-->
										<ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0">
											...
											<!--end::Item-->
											<!--begin::Item-->
											<li class="breadcrumb-item text-muted">{{ __('general.requests_logs') }}</li>
											<!--end::Item-->
										</ul>
										<!--end::Breadcrumb-->
									</div>
									...
									<div class="d-flex align-items-center gap-2 gap-lg-3">
										<form id="delete-form-{{ 1 }}" action="{{ route('deleteRequestsLogs') }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" 
										...
										<a href="{{route('indexRequests')}}" class="btn btn-flex btn-primary h-40px fs-7 fw-bold">{{ __('general.today') }}</a>
										<a href="{{route('indexRequestsAll')}}" class="btn btn-flex btn-info h-40px fs-7 fw-bold">{{ __('general.all') }}</a>
									...
									<div class="card-header align-items-center py-5 gap-2 gap-md-5">
										...
										<div class="card-title">
											...
											<div class="d-flex align-items-center position-relative my-1">
												...
												<input type="text" data-kt-table="requestlogs_table" data-kt-filter="search"
													class="form-control form-control-solid w-250px ps-12"
													placeholder="{{ __('general.search_keyword') }}" />
											</div>
											<!--end::Search-->
											<!--begin::Export buttons-->
											<div id="requestlogs_table_export" class="d-none"></div>
											<!--end::Export buttons-->
										</div>
										<!--end::Card title-->
										<!--begin::Card toolbar-->
										<div class="card-toolbar flex-row-fluid justify-content-end gap-5">
											...
											<div class="w-150px">
												...
												<select class="form-select form-select-solid" data-control="select2"
													...
													data-kt-table="requestlogs_table" data-kt-filter="status">
													<option></option>
													<option value="all">{{ __('general.all') }}</option>
													<option value="this_day">{{ __('general.daily') }}</option>
													<option value="this_month">{{ __('general.monthly') }}</option>
=												</select>
												<!--end::Select2-->
											</div>
											<!--end::Filter-->
											...
											<!--begin::Menu-->
											<div id="requestlogs_table_export_menu"
										...
										<table class="table table-hover align-middle table-row-dashed fs-6 kt-datatable gy-3"
											...
											<tbody class="fw-semibold text-gray-800 ">
												...
												<tr>
													...
													<td class="text-start">{{ __('general.' . strtolower($log['action_type'])) }}</td>
													<td class="text-start">{{ $log['action'] }}</td>
													<td class="text-start">{{ $log['date'] }}</td>
												</tr>
												@endforeach
											</tbody>
										</table>
	...
	@include('log._get_monthly_logs', ['route' => 'getMonthlyLogsRequests'])

	<script>
    	document.addEventListener('DOMContentLoaded', function () {
    	    const table = document.querySelector('#requestlogs_table');
    	    if (table) {
    	        $(table).DataTable({
