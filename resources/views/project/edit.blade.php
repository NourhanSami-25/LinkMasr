...
<head>
	...
	<title>Link Masr | Update Project</title>
					...
					<div class="d-flex flex-column flex-column-fluid">
						...
						<div id="kt_app_content" class="app-content flex-column-fluid">
							...
							<div id="kt_app_content_container" class="app-container container-fluid">
								<!--begin::Form-->
								<form action="{{route('projects.update', $project->id)}}" method="POST" class="form d-flex flex-column flex-lg-row"
														...
														<div class="card-body pt-0">
															...
															<div class="d-flex flex-wrap gap-5 mb-5">
																...
																<div class="fv-row w-100 flex-md-root">
																	...
																	<!--end::Label-->
																	<!--begin::Input-->
																	<input type="text" name="subject" data-label="{{ __('general.subject') }}"  class="form-control mb-2" value="{{ $project->subject }}" data-required="true" data-minlength="5" data-maxlength="255"/>
																	<!--end::Input-->
																</div>
																<!--end::Input group-->
																<!--begin::Input group-->
																...
																<div class="fv-row w-100 flex-md-root">
																	...
																	<select class="form-select mb-2" name="status" data-label="{{ __('general.status') }}"
																		...
																		data-placeholder="{{ __('general.select_an_option') }}" data-required="true">
																		<option></option>
																		<option value="in_progress" {{ $project->status === 'in_progress' ? 'selected' : '' }}>{{ __('general.in_progress') }}</option>
																		<option value="on_hold" {{ $project->status === 'on_hold' ? 'selected' : '' }}>{{ __('general.on_hold') }}</option>
																		<option value="not_started" {{ $project->status === 'not_started' ? 'selected' : '' }}>{{ __('general.not_started') }}</option>
																		<option value="completed" {{ $project->status === 'completed' ? 'selected' : '' }}>{{ __('general.completed') }}</option>
																	</select>
																	<!--end::Select2-->
																</div>
																<!--end::Input group-->
															...
															<div class="d-flex flex-wrap gap-5 mb-5">
																	...
																	<select class="form-select mb-2" name="assignees[]" data-label="{{ __('general.assignees') }}"
																	    ...
																	    @foreach($users as $user)
																	        <option value="{{ $user->id }}"
																	            @if(in_array($user->id, json_decode($project->assignees ?? '[]', true)))
																	                selected
																	            @endif>
																	            {{ $user->name }}
																	        </option>
																...
																<div class="fv-row w-100 flex-md-root">
																	...
																	<select class="form-select mb-2" name="followers[]" data-label="{{ __('general.followers') }}"
																		...
																		@foreach($users as $user)
																	        <option value="{{ $user->id }}"
																	            @if(in_array($user->id, json_decode($project->followers ?? '[]', true)))
																	                selected
																	            @endif>
																	            {{ $user->name }}
																	        </option>
															...
															<div class="mb-5 mt-5">
																...
																<!--end::Label-->
																<!--begin::Editor-->
																<textarea class="form-control mb-2" name="description" data-label="{{ __('general.description') }}" maxlength="1024">{{ $project->description }}</textarea>
																<!--end::Editor-->
																<!--begin::Description-->
																<div class="text-muted fs-7">{{ __('general.optional_message') }}</div>
																<!--end::Description-->
														...
														<div class="card-body pt-0">
															...
															<div class="d-flex flex-wrap gap-5 mb-5">
																...
																<div class="fv-row w-100 flex-md-root">
																	...
																	<!--end::Label-->
																	<!--begin::Input-->
																	<input class="flatpickr-date form-control mb-2" name="date" data-label="{{ __('general.start_date') }}" value="{{ $project->date }}" placeholder="{{ __('general.select_a_date') }}" data-type="dateTime" data-required="true"/>
																	<!--end::Input-->
																</div>
																<!--end::Input group-->
																<!--begin::Input group-->
																<div class="fv-row w-100 flex-md-root">
																	...
																	<!--end::Label-->
																	<!--begin::Input-->
																	<input class="flatpickr-date form-control mb-2" name="due_date" data-label="{{ __('general.due_date') }}" value="{{ $project->due_date }}" placeholder="{{ __('general.select_a_date') }}" data-type="dateTime" data-required="true"/>
																	<!--end::Input-->
																</div>
																<!--end::Input group-->
																<!--begin::Input group-->
																<div class="fv-row w-100 flex-md-root">
																	...
																	<select class="form-select mb-2" name="billing_type" data-label="{{ __('general.billing_type') }}"
																		...
																		<option></option>
																		<option value="fixed_rate" {{ $project->billing_type === 'fixed_rate' ? 'selected' : '' }}>{{ __('general.fixed_rate') }}</option>
																		<option value="task_hours" {{ $project->billing_type === 'task_hours' ? 'selected' : '' }}>{{ __('general.task_hours') }}</option>
																	</select>
																	<!--end::Select2-->
																	<!--begin::Description-->
																		<div class="text-muted fs-7">{{ __('general.optional_message') }}</div>
