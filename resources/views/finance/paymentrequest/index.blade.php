...
<head>
	...
	<title>Link Masr | PaymentRequests</title>
...
<body id="kt_app_body" data-kt-app-header-fixed="true" data-kt-app-header-fixed-mobile="true"
								...
								<div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
									...
									<div class="page-title d-flex flex-column justify-content-center gap-1 me-3">
										...
										<h1
											...
											{{ __('general.all_paymentRequests') }} </h1>
										<!--end::Title-->
										<!--begin::Breadcrumb-->
									...
									<div class="card-header align-items-center py-5 gap-2 gap-md-5">
										...
										<div class="card-title">
											...
											<div class="d-flex align-items-center position-relative my-1">
												...
												<input type="text" data-kt-table="paymentrequests_table" data-kt-filter="search"
													class="form-control form-control-solid w-250px ps-12"
													placeholder="{{ __('general.search_keyword') }}" />
											</div>
											<!--end::Search-->
											<!--begin::Export buttons-->
											<div id="paymentrequests_table_export" class="d-none"></div>
											<!--end::Export buttons-->
										</div>
										<!--end::Card title-->
										<!--begin::Card toolbar-->
										<div class="card-toolbar flex-row-fluid justify-content-end gap-5">
											...
											<div class="w-150px">
												...
												<select class="form-select form-select-solid" data-control="select2"
													...
													data-kt-table="paymentrequests_table" data-kt-filter="status">
													<option></option>
													<option value="all">{{ __('general.all') }}</option>
													<option value="{{ __('general.draft') }}">{{ __('general.draft') }}</option>													
													<option value="{{ __('general.sent') }}">{{ __('general.sent') }}</option>													
													<option value="{{ __('general.accepted') }}">{{ __('general.accepted') }}</option>													
													<option value="{{ __('general.expired') }}">{{ __('general.expired') }}</option>													
													<option value="{{ __('general.declined') }}">{{ __('general.declined') }}</option>												
												</select>
												<!--end::Select2-->
											</div>
											<!--end::Filter-->
											...
											<!--begin::Menu-->
											<div id="paymentrequests_table_export_menu"
									...
									<div class="card-body pt-0">
										...
										<table class="table table-hover align-middle table-row-dashed fs-6 kt-datatable gy-3"
											...
											<tbody class="fw-semibold text-gray-800">
												...
												@include('finance.paymentrequest._create_payment', ['paymentRequest' => $paymentRequest, 'modalId' => "kt_modal_new_target_create_payment_{$paymentRequest->id}"])
												<tr>
													<td class="text-start">{{$paymentRequest->id}}</td>
													<td class="text-start">
														@hasAccess('finance','details')
														<a href="{{route('paymentRequests.show' , $paymentRequest->id)}}" class="text-primary-900 text-hover-primary">
														@endhasAccess
															{{$paymentRequest->number}}</a>
													</td>
													<td class="text-start">{{$paymentRequest->client_name}}</td>
													<td class="text-start">
														<div class="badge 
															{{ $paymentRequest->status === 'draft' ? 'badge-light-info' : '' }}
															{{ $paymentRequest->status === 'sent' ? 'badge-light-primary' : '' }}
															{{ $paymentRequest->status === 'accepted' ? 'badge-light-success' : '' }}
															{{ $paymentRequest->status === 'expired' ? 'badge-light-warning' : '' }}
															{{ $paymentRequest->status === 'declined' ? 'badge-light-danger' : '' }}">
															{{ __('general.' . $paymentRequest->status) }}
														</div>
													</td>		
													...
													<td class="text-start">{{date('Y-m-d', strtotime($paymentRequest->due_date))}}</td>
													<td class="text-end">
														<a href="#"
														...
														<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
															...
															<!--begin::Menu item-->
															<div class="menu-item px-3">
																<a href="{{route('paymentRequest_show_pdf' , $paymentRequest->id)}}"
																	class="menu-link px-3" target="_blank">{{ __('general.print&download') }}</a>
															</div>
															<!--end::Menu item-->
															@endhasAccess
															...
															<!--begin::Menu item-->
															<div class="menu-item px-3">
																<a href="#" data-bs-toggle="modal" data-bs-target="#kt_modal_new_target_create_payment_{{ $paymentRequest->id }}"
																	class="menu-link px-3">{{ __('general.create_payment') }}</a>
															</div>
															<!--end::Menu item-->
															...
															<div class="menu-item px-3">
																<form id="delete-form-{{ $paymentRequest->id }}" action="{{ route('paymentRequests.destroy', $paymentRequest->id) }}" method="POST">
																	...
																	@method('DELETE')
																	<div class="menu-item">
																		<button type="button" onclick="showConfirmation('{{ addslashes($paymentRequest->subject) }}', '{{ $paymentRequest->id }}');" class="dropdown-item menu-link fw-bold text-danger">{{ __('general.delete') }}</button>
																	</div>
																</form>
															</div>
															<!--end::Menu item-->
	...
	<script>
    	document.addEventListener('DOMContentLoaded', function () {
    	    const table = document.querySelector('#paymentrequests_table');
    	    if (table) {
    	        $(table).DataTable({
