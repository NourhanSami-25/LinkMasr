...
<head>
	...
	<title>Link Masr | Update PaymentRequest</title>
								...
								<div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
									...
									<div class="page-title d-flex flex-column justify-content-center gap-1 me-3">
										...
										<h1 class="page-heading d-flex flex-column justify-content-center text-gray-900 fw-bold fs-1 m-0">
											{{ __('general.update_paymentRequest') }}</h1>
										<!--end::Title-->
										<!--begin::Breadcrumb-->
									...
									<div class="flex-lg-row-fluid mb-10 mb-lg-0 me-lg-7 me-xl-10">
										...
										<div class="card">
											...
											<div class="card-body p-12">
												<!--begin::Form-->
												<form  id="kt_invoice_form" action="{{route('paymentRequests.update' , $paymentRequest->id)}}" method="POST" enctype="multipart/form-data">
													@csrf											
                                                    @method('PUT')		
													<!--begin::Wrapper-->
													<div class="d-flex flex-column align-items-start flex-xxl-row">
														...
														<div class="d-flex align-items-center flex-equal fw-row me-4 order-2"
															...
															<div
																...
																<input 
																	...
																	placeholder="{{ __('general.select_date') }}" name="date" value="{{ $paymentRequest->date }}" data-type="date" data-required="true"/>
																<!--end::Datepicker-->
																<!--begin::Icon-->
														...
														<div class="d-flex flex-center flex-equal fw-row text-nowrap order-1 order-xxl-2 me-4"
															...
															<input name="number" class="form-control form-control-flush fw-bold text-gray-800 fs-3 w-125px" value="{{$paymentRequest->number}}" placehoder="2021001..."data-type="number" data-required="true"/>
														</div>
														<!--end::Input group-->
														<!--begin::Input group-->
														<div class="d-flex align-items-center justify-content-end flex-equal order-3 fw-row"
															...
															<div class="position-relative d-flex align-items-center w-150px">
																...
																<input class="flatpickr-date form-control form-control-transparent fw-bold pe-5" 
																	placeholder="{{ __('general.select_date') }}" name="due_date" value="{{ $paymentRequest->due_date }}" data-type="date"/>
																<!--end::Datepicker-->
																<!--begin::Icon-->
													...
													<div class="mb-0">
																...
																<div class="mb-5">
																	<select name="client_id" id="clientSelect"
																		...
																		@foreach($clients as $client)
																			@if($client->address)
																				<option value="{{$client->id}}" data-address="{{$client->address->street_name}}" {{ old('client_id', $paymentRequest->client_id ?? '') == $client->id ? 'selected' : '' }}>{{$client->name}}
                                                                                </option>
																			@else
																				<option value="{{$client->id}}" data-address="address" {{ old('client_id', $paymentRequest->client_id ?? '') == $client->id ? 'selected' : '' }}>{{$client->name}}</option>
																			@endif
																		@endforeach
																	</select>
																</div>
																	...
																	<select name="task_id" aria-label="Select a Timezone"
																		...
																		@foreach($tasks as $task)
																	        <option value="{{ $task->id }}" data-client-id="{{ $task->client_id }}"
																	            {{ old('task_id', $paymentRequest->task_id ?? '') == $task->id ? 'selected' : '' }}>
																	            {{ $task->subject }}
																	        </option>
																	    @endforeach
																	</select>
															...
															<div class="col-lg-6">
																...
																<div class="mb-5">
																	<input type="text" id="clientAddress" name="billing_address"
																		class="form-control form-control-solid"
																		placeholder="{{ __('general.client_address') }}" value="{{$paymentRequest->billing_address}}"/>
																</div>
																<!--end::Input group-->
															</div>
															<!--end::Col-->
															...
															<div class="col-lg-12">
																...
																<div class="mb-5">
																	<textarea name="description"
																		...
																		placeholder="{{ __('general.description') }}">{{$paymentRequest->description}}</textarea>
																</div>
																<!--end::Input group-->
															</div>
															<!--end::Col-->
															...
															<table class="table g-5 gs-0 mb-0 fw-bold text-gray-700"
																...
																<tfoot>
																	...
																	<tr colspan="2" class="border-bottom border-bottom-dashed ps-0">
																		...
																		<th>
																			<input class="form-control form-control-solid w-75" type="number" min="0" max="100" name="tax" placeholder="0" value="{{$paymentRequest->tax}}" data-kt-element="invoice-tax" />
																		</th>
																		<th colspan="2" class="border-bottom border-bottom-dashed text-end">$ <span data-kt-element="invoice-tax-field">{{$paymentRequest->total_tax}}</span></th>
																	</tr>
																	...
																	<tr colspan="2" class="border-bottom border-bottom-dashed ps-0">
																		...
																		<th>
																			<input class="form-control form-control-solid w-75" type="number" min="0" max="100" name="discount" placeholder="0" value="{{$paymentRequest->discount}}" data-kt-element="invoice-discount" />
																		</th>
																		<th colspan="2" class="border-bottom border-bottom-dashed text-end">$ <span data-kt-element="invoice-discount-field">{{$paymentRequest->percentage_discount_value}}</span></th>
																	</tr>
																	...
																	<tr colspan="2" class="border-bottom border-bottom-dashed ps-0">
																		...
																		<th>
																			<input class="form-control form-control-solid w-75" type="number" min="0" max="99999999" name="fixed_discount" placeholder="0" value="{{$paymentRequest->fixed_discount}}" data-kt-element="invoice-fixed-discount" />
																		</th>
																		<th colspan="2" class="border-bottom border-bottom-solid text-end">$ <span data-kt-element="invoice-fixed-discount-field">{{$paymentRequest->fixed_discount}}</span></th>
																	</tr>
														...
														<div class="row mb-10">
															<div class="col">
																...
																<textarea name="client_note" class="form-control form-control-solid" rows="3" placeholder="{{ __('general.note_info') }}">{{$paymentRequest->client_note}}</textarea>
															</div>
															<div class="col">
																<label class="form-label fs-6 fw-bold text-gray-700">{{ __('general.admin_note') }}</label>
															<textarea name="admin_note" class="form-control form-control-solid" rows="3" placeholder="{{ __('general.note_info') }}">{{$paymentRequest->admin_note}}</textarea>
															</div>
														</div>
														<!--end::Notes-->
														{{-- <!--begin::bank details-->
													...
													<select name="project_id" aria-label="Select a Timezone"
														...
														@foreach($projects as $project)
														    <option value="{{ $project->id }}" data-client-id="{{ $project->client_id }}" 
														        {{ old('project_id', $paymentRequest->project_id ?? '') == $project->id ? 'selected' : '' }}>
														        {{ $project->subject }}
														    </option>
														@endforeach
													</select>
													...
													<select name="sale_agent" aria-label="Select a Timezone"
														...
														@foreach($users as $user)
														    <option value="{{ $user->id }}" 
														        {{ old('sale_agent', $paymentRequest->sale_agent ?? '') == $user->id ? 'selected' : '' }}>
														        {{ $user->name }}
														    </option>
														@endforeach
													</select>
													...
													<div class="col mb-5">
														...
														<select name="currency" aria-label="Select a Timezone" id="clientCurrency"
														    ...
														    <option value="{{ $currency->code }}" 
														        {{ old('currency', $paymentRequest->currency ?? '') == $currency->code ? 'selected' : '' }}>
														        {{ $currency->code }}
														    </option>
														@endforeach
														</select>
												...
												<div class="row">
													<div class="col mb-5">
														...
														<select name="discount_type" aria-label="Select discount type"
                                                            ...
                                                            <option value="before_tax" {{ $paymentRequest->discount_type === 'before_tax' ? 'selected' : '' }}>{{ __('general.before_tax') }}</option>
                                                            <option value="after_tax" {{ $paymentRequest->discount_type === 'after_tax' ? 'selected' : '' }}>{{ __('general.after_tax') }}</option>
														</select>
														<!--end::Select-->
													</div>
												</div>
												<div class="row">
													<div class="col mb-5">
														...
														<select name="discount_amount_type" aria-label="Select discount type"
                                                            ...
                                                            <option value="percentage" {{ $paymentRequest->discount_amount_type === 'percentage' ? 'selected' : '' }}>{{ __('general.percentage') }}</option>
                                                            <option value="fixed_amount" {{ $paymentRequest->discount_amount_type === 'fixed_amount' ? 'selected' : '' }}>{{ __('general.fixed_amount') }}</option>
														</select>
														<!--end::Select-->
													</div>
												</div>
												...
												<div class="row">
													<div class="col mb-5">
														...
														<select name="status" aria-label="Select status"
															data-control="select2" data-hide-search="true"
															class="form-select form-select-solid">
                                                            <option value="draft" {{ $paymentRequest->status === 'draft' ? 'selected' : '' }}>{{ __('general.draft') }}</option>
                                                            <option value="sent" {{ $paymentRequest->status === 'sent' ? 'selected' : '' }}>{{ __('general.sent') }}</option>
                                                            <option value="accepted" {{ $paymentRequest->status === 'accepted' ? 'selected' : '' }}>{{ __('general.accepted') }}</option>
                                                            <option value="expired" {{ $paymentRequest->status === 'expired' ? 'selected' : '' }}>{{ __('general.expired') }}</option>
                                                            <option value="declined" {{ $paymentRequest->status === 'declined' ? 'selected' : '' }}>{{ __('general.declined') }}</option>
														</select>
														<!--end::Select-->
													</div>
												</div>
