...
<head>
	...
	<title>Link Masr | Invoices</title>
...
<body id="kt_app_body" data-kt-app-header-fixed="true" data-kt-app-header-fixed-mobile="true"
									...
									<div class="card-header align-items-center py-5 gap-2 gap-md-5">
										...
										<div class="card-title">
											...
											<div class="d-flex align-items-center position-relative my-1">
												...
												<input type="text" data-kt-table="invoices_table" data-kt-filter="search"
													class="form-control form-control-solid w-250px ps-12"
													placeholder="{{ __('general.search_keyword') }}" />
											</div>
											<!--end::Search-->
											<!--begin::Export buttons-->
											<div id="invoices_table_export" class="d-none"></div>
											<!--end::Export buttons-->
										</div>
										<!--end::Card title-->
										<!--begin::Card toolbar-->
										<div class="card-toolbar flex-row-fluid justify-content-end gap-5">
											...
											<div class="w-150px">
												...
												<select class="form-select form-select-solid" data-control="select2"
													...
													data-kt-table="invoices_table" data-kt-filter="status">
													<option></option>
											...
											<!--begin::Menu-->
											<div id="invoices_table_export_menu"
									...
									<div class="card-body pt-0">
										...
										<table class="table table-hover align-middle table-row-dashed fs-6 kt-datatable gy-3"
											...
											<tbody class="fw-semibold text-gray-800">
												...
												@include('finance.invoice._create_payment', ['invoice' => $invoice, 'modalId' => "kt_modal_new_target_create_payment_{$invoice->id}"])
												<tr>
													<td class="text-start">{{$invoice->id}}</td>
													<td class="text-start">
														@hasAccess('finance','details')
														<a href="{{route('invoices.show' , $invoice->id)}}" class="text-primary-900 text-hover-primary">
														@endhasAccess
															{{$invoice->number}}</a>
													</td>
													<td class="text-start">{{$invoice->client_name}}</td>
													<td class="text-start">
														<div class="badge 
															{{ $invoice->status === 'paid' ? 'badge-light-success' : '' }}
															{{ $invoice->status === 'unpaid' ? 'badge-light-danger' : '' }}
															{{ $invoice->status === 'overdue' ? 'badge-light-warning' : '' }}
															{{ $invoice->status === 'partially' ? 'badge-light-primary' : '' }}
															{{ $invoice->status === 'draft' ? 'badge-light-info' : '' }}">
															{{ __('general.' . $invoice->status) }}
														</div>
													</td>		
													<td class="text-start">{{$invoice->total}}</td>
													<td class="text-start">{{$invoice->currency}}</td>
													<td class="text-start">{{date('Y-m-d', strtotime($invoice->date))}}</td>
													<td class="text-start">{{date('Y-m-d', strtotime($invoice->due_date))}}</td>
													<td class="text-end">
														<a href="#"
														...
														<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
															...
															<!--begin::Menu item-->
															<div class="menu-item px-3">
																<a href="{{route('invoice_show_pdf' , $invoice->id)}}"
															...
															<!--begin::Menu item-->
															<div class="menu-item px-3">
																<a href="#" data-bs-toggle="modal" data-bs-target="#kt_modal_new_target_create_payment_{{ $invoice->id }}"
															...
															<div class="menu-item px-3">
																<form id="delete-form-{{ $invoice->id }}" action="{{ route('invoices.destroy', $invoice->id) }}" method="POST">
																	...
																	@method('DELETE')
																	<div class="menu-item">
																		<button type="button" onclick="showConfirmation('{{ addslashes($invoice->subject) }}', '{{ $invoice->id }}');" class="dropdown-item menu-link fw-bold text-danger">{{ __('general.delete') }}</button>
																	</div>
																</form>
															</div>
															<!--end::Menu item-->
	...
	<script>
    	document.addEventListener('DOMContentLoaded', function () {
    	    const table = document.querySelector('#invoices_table');
    	    if (table) {
    	        $(table).DataTable({
