...
@section('title')
    {{ __('general.edit_invoice') }} #{{ $invoice->number }}
@endsection
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item text-muted">{{ $invoice->number }}</li>
@endsection

@section('content')
    <div class="card card-flush border-0">
        <div class="card-body">
            <form id="kt_invoice_form" action="{{ route('invoices.update', $invoice->id) }}" method="POST">
                @csrf
                @method('PUT')
                                    ...
                                    <div class="col-lg-4">
                                        ...
                                        <select name="project_id" class="form-select form-select-solid"
                                            ...
                                            @foreach($projects as $project)
                                                <option value="{{ $project->id }}" {{ $invoice->project_id == $project->id ? 'selected' : '' }}>{{ $project->subject }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-lg-4">
                                        ...
                                        <select name="task_id" class="form-select form-select-solid" data-control="select2">
                                            ...
                                            @foreach($tasks as $task)
                                                <option value="{{ $task->id }}" {{ $invoice->task_id == $task->id ? 'selected' : '' }}>{{ $task->subject }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-lg-4">
                                        ...
                                        <select name="pymentRequest_id" class="form-select form-select-solid"
                                            ...
                                            @foreach($paymentRequests as $pr)
                                                <option value="{{ $pr->id }}" {{ $invoice->pymentRequest_id == $pr->id ? 'selected' : '' }}>#{{ $pr->number }} ({{ $pr->client_name }})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="mb-5">
                                    ...
                                    <select name="currency" id="clientCurrency" class="form-select form-select-solid"
                                        ...
                                        @foreach($currencies as $currency)
                                            <option value="{{ $currency->code }}" {{ $invoice->currency == $currency->code ? 'selected' : '' }}>{{ $currency->code }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="mb-5">
