	...
	<div class="modal-dialog modal-dialog-centered mw-650px">
		...
		<div class="modal-content rounded">
			...
			<div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
				<!--begin:Form-->
				<form action="{{ route('convert_invoice_to_pyment' , $invoice->id) }}" method="GET" id="kt_modal_new_target_form" class="form" enctype="multipart/form-data">
					@csrf
                    <!--begin::Heading-->
                    ...
                    <input class="form-control form-control-solid"  type="number" name="model_id" value="{{$invoice->id}}" hidden/>
                    <input class="form-control form-control-solid"  type="text" name="model_type" value="{{ get_class($invoice) }}" hidden/>
                    <!--end::Input group - Hidden-->

                    <!--begin::Input group-->
					<div class="d-flex flex-column mb-8">
						<label class="fs-6 fw-semibold mb-2 fs-3">{{ __('general.Payment_total') }}</label>
						<input class="form-control form-control-solid fs-2" type="decimal" name="total" value="{{ $invoice->total }}" data-required="true"/>
					</div>
					<!--end::Input group-->
