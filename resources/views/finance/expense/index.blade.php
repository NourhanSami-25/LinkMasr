...
<head>
	...
	<title>Link Masr | Expenses</title>
									...
									<div class="card-header align-items-center py-5 gap-2 gap-md-5">
										...
										<div class="card-title">
											...
											<div class="d-flex align-items-center position-relative my-1">
												...
												<input type="text" data-kt-table="expenses_table" data-kt-filter="search"
													class="form-control form-control-solid w-250px ps-12"
													placeholder="{{ __('general.search_keyword') }}" />
											</div>
											<!--end::Search-->
											<!--begin::Export buttons-->
											<div id="expenses_table_export" class="d-none"></div>
											<!--end::Export buttons-->
										</div>
										<!--end::Card title-->
										<!--begin::Card toolbar-->
										<div class="card-toolbar flex-row-fluid justify-content-end gap-5">
											...
											<div class="w-150px">
												...
												<select class="form-select form-select-solid" data-control="select2"
													...
													data-kt-table="expenses_table" data-kt-filter="status">
													<option></option>
											...
											<!--begin::Menu-->
											<div id="expenses_table_export_menu"
										...
										<table class="table table-hover align-middle table-row-dashed fs-6 kt-datatable gy-3"
											...
											<tbody class="fw-semibold text-gray-800">
												...
												<tr>
													...
													<td class="text-start">
														@hasAccess('expense','details')
														<a href="{{route('expenses.show' , $expense->id)}}" class="text-primary-900 text-hover-primary">
														@endhasAccess
															{{$expense->number}}</a>
													</td>
													<td class="text-start">{{$expense->client_name}}</td>
													<td class="text-start">
														<div class="badge 
															{{ $expense->status === 'paid' ? 'badge-light-success' : '' }}
															{{ $expense->status === 'unpaid' ? 'badge-light-danger' : '' }}
															{{ $expense->status === 'overdue' ? 'badge-light-warning' : '' }}
															{{ $expense->status === 'partially' ? 'badge-light-primary' : '' }}
															{{ $expense->status === 'draft' ? 'badge-light-info' : '' }}">
															{{ __('general.' . $expense->status) }}
														</div>
													</td>		
													...
													<td class="text-start">{{ __('general.'.$expense->type) }}</td>
													<td class="text-end">
														<a href="#"
														...
														<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
															...
															<!--begin::Menu item-->
															<div class="menu-item px-3">
																<a href="{{route('expense_show_pdf' , $expense->id)}}"
																	class="menu-link px-3" target="_blank">{{ __('general.print&download') }}</a>
															</div>
															<!--end::Menu item-->
															@endhasAccess
															@hasAccess('expense','modify')
															<!--begin::Menu item-->
															<div class="menu-item px-3">
															...
															<div class="menu-item px-3">
																<form id="delete-form-{{ $expense->id }}" action="{{ route('expenses.destroy', $expense->id) }}" method="POST">
																	...
																	@method('DELETE')
																	<div class="menu-item">
																		<button type="button" onclick="showConfirmation('{{ addslashes($expense->subject) }}', '{{ $expense->id }}');" class="dropdown-item menu-link fw-bold text-danger">{{ __('general.delete') }}</button>
																	</div>
																</form>
															</div>
															<!--end::Menu item-->
