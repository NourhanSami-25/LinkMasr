...
<head>
	...
	<title>Link Masr | Credit Notes</title>
...
<body id="kt_app_body" data-kt-app-header-fixed="true" data-kt-app-header-fixed-mobile="true"
									...
									<div class="card-header align-items-center py-5 gap-2 gap-md-5">
										...
										<div class="card-title">
											...
											<div class="d-flex align-items-center position-relative my-1">
												...
												<input type="text" data-kt-table="creditnotes_table" data-kt-filter="search"
													class="form-control form-control-solid w-250px ps-12"
													placeholder="{{ __('general.search_keyword') }}" />
											</div>
											<!--end::Search-->
											<!--begin::Export buttons-->
											<div id="creditnotes_table_export" class="d-none"></div>
											<!--end::Export buttons-->
										</div>
										<!--end::Card title-->
										<!--begin::Card toolbar-->
										<div class="card-toolbar flex-row-fluid justify-content-end gap-5">
											...
											<div class="w-150px">
												...
												<select class="form-select form-select-solid" data-control="select2"
													...
													data-kt-table="creditnotes_table" data-kt-filter="status">
													<option></option>
													<option value="all">{{ __('general.all') }}</option>
													<option value="{{ __('general.paid') }}">{{ __('general.paid') }}</option>
													<option value="{{ __('general.partially_paid') }}">{{ __('general.partially_paid') }}</option>
													<option value="{{ __('general.unpaid') }}">{{ __('general.unpaid') }}</option>
													<option value="{{ __('general.overdue') }}">{{ __('general.overdue') }}</option>
											...
											<!--begin::Menu-->
											<div id="creditnotes_table_export_menu"
									...
									<div class="card-body pt-0">
										...
										<table class="table table-hover align-middle table-row-dashed fs-6 kt-datatable gy-3"
											...
											<tbody class="fw-semibold text-gray-800">
												...
												<tr>
													<td class="text-start">{{$creditNote->id}}</td>
													<td class="text-start">
														@hasAccess('finance','details')
														<a href="{{route('creditNotes.show' , $creditNote->id)}}" class="text-primary-900 text-hover-primary">
														@endhasAccess
															{{$creditNote->number}}</a>
													</td>
													<td class="text-start">{{$creditNote->client_name}}</td>
													<td class="text-start">
														<div class="badge 
															{{ $creditNote->status === 'paid' ? 'badge-light-success' : '' }}
															{{ $creditNote->status === 'unpaid' ? 'badge-light-danger' : '' }}
															{{ $creditNote->status === 'partially_paid' ? 'badge-light-secondary' : '' }}
															{{ $creditNote->status === 'overdue' ? 'badge-light-warning' : '' }}
															{{ $creditNote->status === 'partially' ? 'badge-light-primary' : '' }}
															{{ $creditNote->status === 'draft' ? 'badge-light-info' : '' }}">
															{{ __('general.' . $creditNote->status) }}
														</div>
													</td>		
													<td class="text-start">{{$creditNote->total}}</td>
													<td class="text-start">{{$creditNote->payment_currency}}</td>
													<td class="text-start">{{date('Y-m-d', strtotime($creditNote->date))}}</td>
													<td class="text-start">{{date('Y-m-d', strtotime($creditNote->due_date))}}</td>
													<td class="text-end">
														<a href="#"
														...
														<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
															...
															<!--begin::Menu item-->
															<div class="menu-item px-3">
																<a href="{{route('creditNote_show_pdf' , $creditNote->id)}}"
															...
															<!--begin::Menu item-->
															<div class="menu-item px-3">
																<a href="{{route('creditNotes.edit', $creditNote->id)}}"
															...
															<!--begin::Menu item-->
															<div class="menu-item px-3">
																<form id="delete-form-{{ $creditNote->id }}" action="{{ route('creditNotes.destroy', $creditNote->id) }}" method="POST">
																	@csrf
																	@method('DELETE')
																	<div class="menu-item">
																		<button type="button" onclick="showConfirmation('{{ addslashes($creditNote->subject) }}', '{{ $creditNote->id }}');" class="dropdown-item menu-link fw-bold text-danger">{{ __('general.delete') }}</button>
																	</div>
																</form>
															</div>
															<!--end::Menu item-->
	...
	<script>
    	document.addEventListener('DOMContentLoaded', function () {
    	    const table = document.querySelector('#creditnotes_table');
    	    if (table) {
    	        $(table).DataTable({
