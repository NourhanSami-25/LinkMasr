...
<head>
	...
	<title>Link Masr | Announcements</title>
								...
								<div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
									...
									<div class="page-title d-flex flex-column justify-content-center gap-1 me-3">
										...
										<h1
											...
											{{ __('general.all_announcements') }} </h1>
										<!--end::Title-->
										<!--begin::Breadcrumb-->
									...
									<div class="card-header align-items-center py-5 gap-2 gap-md-5">
										...
										<div class="card-title">
											...
											<div class="d-flex align-items-center position-relative my-1">
												...
												<input type="text" data-kt-table="announcements_table" data-kt-filter="search"
													class="form-control form-control-solid w-250px ps-12"
													placeholder="{{ __('general.search_keyword') }}" />
											</div>
											<!--end::Search-->
											<!--begin::Export buttons-->
											<div id="announcements_table_export" class="d-none"></div>
											<!--end::Export buttons-->
										</div>
										<!--end::Card title-->
										<!--begin::Card toolbar-->
										<div class="card-toolbar flex-row-fluid justify-content-end gap-5">
											...
											<div class="w-150px">
												...
												<select class="form-select form-select-solid" data-control="select2"
													...
													data-kt-table="announcements_table" data-kt-filter="status">
													<option></option>
											...
											<!--begin::Menu-->
											<div id="announcements_table_export_menu"
										...
										<table class="table table-hover align-middle table-row-dashed fs-6 kt-datatable gy-3"
											...
											<thead>
												<tr class=" text-gray-500 fw-bold fs-7 text-uppercase gs-0">
													...
													<th class="text-start min-w-50px">{{ __('general.subject') }}</th>
													<th class="text-start min-w-150px">{{ __('general.message') }}</th>
													<th class="text-start min-w-100px">{{ __('general.status') }}</th>
													<th class="text-start min-w-100px">{{ __('general.expire_after') }}</th>
											...
											<tbody class="fw-semibold text-gray-800">
												...
												<tr>
													<td class="text-start">{{$announcement->id}}</td>
													<td class="text-start">
														@hasAccess('announcement','details')	
														<a href="{{route('announcements.show', $announcement->id)}}" class="text-primary-900 text-hover-primary">
														@endhasAccess
														{{$announcement->subject}}</a>
													</td>
													<td class="text-start">{{$announcement->message}}</td>
													<td class="text-start">
														<div class="badge 
															{{ $announcement->status === 'active' ? 'badge-light-primary' : '' }}
															{{ $announcement->status === 'disabled' ? 'badge-light-danger' : '' }}">
															{{ __('general.' . $announcement->status) }}
														</div>
													</td>	
													<td class="text-start">{{$announcement->expire_after}} {{ __('general.days') }}</td>	
													<td class="text-start">{{ __getUserNameById($announcement->created_by) }}</td>
													<td class="text-start">{{$announcement->created_at}}</td>
													<td class="text-end">
														<a href="#"
														...
														<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
															...
															<div class="menu-item px-2">
																<form action="{{ route('announcements.toggleStatus', $announcement->id) }}" method="POST">
																	@csrf
																	@method('PATCH')
																	<div class="menu-item">
																		<button type="submit" class="dropdown-item menu-link fw-bold">{{ $announcement->status == 'active' ?  __('general.disable')  :  __('general.activate')  }}</button>
																	</div>
																</form>
															</div>
															<!--end::Menu item-->
															...
															<!--begin::Menu item-->
															<div class="menu-item px-2">
																<form id="delete-form-{{ $announcement->id }}" action="{{ route('announcements.destroy', $announcement->id) }}" method="POST">
																	@csrf
																	@method('DELETE')
																	<div class="menu-item">
																		<button type="button" onclick="showConfirmation('{{ addslashes($announcement->subject) }}', '{{ $announcement->id }}');" class="dropdown-item menu-link fw-bold text-danger">{{ __('general.delete') }}</button>
																	</div>
																</form>
															</div>
															<!--end::Menu item-->
