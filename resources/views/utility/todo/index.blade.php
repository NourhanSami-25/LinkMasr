...
<head>
	...
	<title>Link Masr | Todo Lists</title>
							...
							<div id="kt_app_toolbar_container"
								...
								<div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
									...
									<div class="d-flex align-items-center gap-2 gap-lg-3">
										<a href="{{route('todos.create')}}" class="btn btn-flex btn-primary h-40px fs-7 fw-bold">{{ __('general.create_new') }}</a>
									...
									<div class="card-header align-items-center py-5 gap-2 gap-md-5">
										...
										<div class="card-title">
											...
											<div class="d-flex align-items-center position-relative my-1">
												...
												<input type="text" data-kt-table="todos_table" data-kt-filter="search"
													class="form-control form-control-solid w-250px ps-12"
													placeholder="{{ __('general.search_keyword') }}" />
											</div>
											<!--end::Search-->
											<!--begin::Export buttons-->
											<div id="todos_table_export" class="d-none"></div>
											<!--end::Export buttons-->
										</div>
										<!--end::Card title-->
										<!--begin::Card toolbar-->
										<div class="card-toolbar flex-row-fluid justify-content-end gap-5">
											...
											<div class="w-150px">
												...
												<select class="form-select form-select-solid" data-control="select2"
													...
													data-kt-table="todos_table" data-kt-filter="status">
													<option></option>
											...
											<!--begin::Menu-->
											<div id="todos_table_export_menu"
										...
										<table class="table table-hover align-middle table-row-dashed fs-6 kt-datatable gy-3"
											...
											<thead>
												<tr class=" text-gray-500 fw-bold fs-7 text-uppercase gs-0">
													...
													<th class="text-start min-w-150px">{{ __('general.description') }}</th>
											...
											<tbody class="fw-semibold text-gray-800">
												...
												<tr>
													...
													<td class="text-start"><a href="{{route('todos.show', $todo->id)}}" class="text-primary-900 text-hover-primary">{{$todo->subject}}</a></td>
													<td class="text-start">{{$todo->description}}</td>
													<td class="text-start">
														<div class="badge 
															{{ $todo->status === 'pending' ? 'badge-light-primary' : '' }}
															{{ $todo->status === 'completed' ? 'badge-light-success' : '' }}">
															{{ __('general.' . $todo->status) }}
														</div>
													</td>		
													<td class="text-start">
														<div class="badge 
															...
															{{  __('general.' . $todo->priority)  }}
														</div>
													</td>
													<td class="text-start">{{ __getUserNameById($todo->user_id) }}</td>
													...
													<td class="text-end">
														...
														<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
															...
															<div class="menu-item px-2">
																<form action="{{ route('todos.toggleStatus', $todo->id) }}" method="POST">
																	@csrf
																	@method('GET')
																	<div class="menu-item">
																		<button type="submit" class="dropdown-item menu-link fw-bold">{{ $todo->status == 'completed' ? __('general.mark_as_pending')  : __('general.mark_as_completed')  }}</button>
																	</div>
																</form>
															</div>
															<!--end::Menu item-->
															...
															<div class="menu-item px-2">
																<form id="delete-form-{{ $todo->id }}" action="{{ route('todos.destroy', $todo->id) }}" method="POST">
																	...
																	@method('DELETE')
																	<div class="menu-item">
																		<button type="button" onclick="showConfirmation('{{ addslashes($todo->subject) }}', '{{ $todo->id }}');" class="dropdown-item menu-link fw-bold text-danger">{{ __('general.delete') }}</button>
																	</div>
																</form>
															</div>
															<!--end::Menu item-->
