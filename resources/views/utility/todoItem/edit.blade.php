	...
	<div class="modal-dialog modal-dialog-centered mw-800px">
		...
		<div class="modal-content rounded">
			...
			<div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
				<!--begin:Form-->
				<form action="{{ route('todoItems.update' , $todoItem->id) }}" method="POST" id="kt_modal_new_target_form" class="form" enctype="multipart/form-data">
					@csrf
					@method('PUT')
					...
					<div class="row g-9 mb-8">
						...
						<div class="col-md-6 fv-row">
							...
							<div class="position-relative d-flex align-items-center">
								<!--begin::Datepicker-->
								<input name="date" value="{{$todoItem->date}}" placeholder="{{ __('general.select_a_date') }}" class="flatpickr-date form-control mb-2"/>
								<!--end::Datepicker-->
							</div>
							<!--end::Input-->
						</div>
						...
						<div class="col-md-6 fv-row">	
							...
							<select class="form-select" data-control="select2"
								...
								<option value="pending" {{ $todoItem->status === 'pending' ? 'selected' : '' }}>{{ __('general.pending') }}</option>
								<option value="completed" {{ $todoItem->status === 'completed' ? 'selected' : '' }}>{{ __('general.completed') }}</option>
							</select>
						</div>
                        <!--end::Col-->
					</div>
					...
					<div class="row g-9 mb-8">
						...
						<div class="col-md-12 fv-row">
						    ...
						    <!--end::Label-->
						    <textarea type="text" class="form-control" name="subject" data-required="true">{{$todoItem->subject}}</textarea>
						</div>
						<!--end::Col-->
					</div>
					<!--end::Input group-->
