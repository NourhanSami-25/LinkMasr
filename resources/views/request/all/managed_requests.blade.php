...
<head>
	...
	<title>Link Masr | Managed Requests</title>
								...
								<div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
									...
									<div class="page-title d-flex flex-column justify-content-center gap-1 me-3">
										...
										<ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0">
											...
											<!--end::Item-->
											<!--begin::Item-->
											<li class="breadcrumb-item text-muted">{{ __('general.department_requests') }}</li>
											<!--end::Item-->
										</ul>
										<!--end::Breadcrumb-->
									</div>
										...
										<table class="table table-hover align-middle table-row-dashed fs-6 kt-datatable gy-3"
											...
											<thead>
												<tr class=" text-gray-500 fw-bold fs-7 text-uppercase gs-0">
													...
													<th class="text-start min-w-100px">{{ __('general.description') }}</th>
											...
											<tbody class="fw-semibold text-gray-800">
												...
												<tr>
													...
													<td class="text-start">{{class_basename($request)}}</td>
													...
													<td class="text-start">{{ \Illuminate\Support\Str::limit($request->description, 30) }}</td>											
													...
													<td class="text-end">
														...
														<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
															...
															<!--begin::Menu item - Approve (only for pending requests)-->
															<div class="menu-item px-2">
																<form action="{{ route('requests.approve', ['type' => $request->type, 'id' => $request->id]) }}" method="POST">
    															    @csrf
																	<div class="menu-item">
    															    	<button type="submit" class="dropdown-item menu-link fw-bold text-success">
																			<i class="ki-outline ki-check-circle fs-5 me-1"></i>{{ __('general.approve') }}
																		</button>
																	</div>
    															</form>
															</div>
															<!--end::Menu item-->
															<!--begin::Menu item - Reject (only for pending requests)-->
															<div class="menu-item px-2">
																<form action="{{ route('requests.reject', ['type' => $request->type, 'id' => $request->id]) }}" method="POST">
    															    @csrf
																	<div class="menu-item">
    															    	<button type="submit" class="dropdown-item menu-link fw-bold text-danger">
																			<i class="ki-outline ki-cross-circle fs-5 me-1"></i>{{ __('general.reject') }}
																		</button>
																	</div>
    															</form>
															</div>
															<!--end::Menu item-->
															@else
															<!--begin::Status info (for already processed requests)-->
															<div class="menu-item px-3">
																<span class="menu-link px-3 text-muted">
																	<i class="ki-outline ki-information-5 fs-5 me-1"></i>{{ __('general.request_already_processed') }}
																</span>
															</div>
															<!--end::Status info-->
															@endif
