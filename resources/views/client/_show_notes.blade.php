<div class="tab-pane fade" id="kt_customer_view_notes" role="tabpanel">
    ...
    <div class="card pt-4 mb-6 mb-xl-9">
        ...
        <div class="card-header border-0">
            ...
            <div class="d-flex align-items-center gap-2 gap-lg-3">
				...
				@endhasAccess
                {{-- <a href="{{route('notes.index' )}}" class="btn btn-flex btn-outline btn-primary h-40px fs-7 fw-bold" target="_blank">{{ __('general.view_all') }}</a> --}}
			</div>
            <!--end::Card toolbar-->
        </div>
        <!--end::Card header-->
            ...
            <table class="table align-middle table-row-dashed kt-datatable gy-5"
                ...
                <thead class="border-bottom border-gray-200 fs-7 fw-bold">
                    <tr class="text-muted text-uppercase gs-0">
                        ...
                        <th class="text-start">{{ __('general.content') }}</th>
                ...
                <tbody class="fs-6 fw-semibold text-gray-800">
                    ...
                    @include('common.note.update_note', ['noteId' => $note->id, 'modalId' => "kt_modal_new_target_update_note_{$note->id}"])
                    <tr>
                        <td class="text-start">{{ $note->id }}</td>
                        <td class="text-start">{{ $note->content }}</td>
                        <td class="text-start">{{ __getUserNameById($note->created_by) }}</td>
                        <td class="text-start">{{ $note->created_at }}</td>
                        <td class="pe-0 text-end">
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                ...
                                <!--begin::Menu item-->
                                <div class="menu-item px-3">
                                    <a href="#" data-bs-toggle="modal" data-bs-target="#kt_modal_new_target_update_note_{{ $note->id }}"
                                ...
                                <div class="menu-item px-3">
                                    <form id="delete-form-{{ $note->id }}" action="{{ route('notes.destroy', $note->id) }}" method="POST">
                                        ...
                                        @method('DELETE')
                                        <div class="menu-item">
                                            <button type="button" onclick="showConfirmation('{{ addslashes($note->subject) }}', '{{ $note->id }}');" class="dropdown-item menu-link fw-bold text-danger">{{ __('general.delete') }}</button>
                                        </div>
                                    </form>
                                </div>
                                <!--end::Menu item-->
...
@include('common.note.create_note', ['item' => $client])
