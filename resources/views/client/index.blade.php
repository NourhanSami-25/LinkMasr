								...
								<div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
									...
									<div class="d-flex align-items-center gap-2 gap-lg-3">
										...
										<a href="{{route('clients.create')}}"
											class="btn btn-flex btn-outline btn-primary h-40px fs-7 fw-bold">{{ __('general.create_new') }}</a>
										@endhasAccess
									...
									<div class="card-header align-items-center py-5 gap-2 gap-md-5">
										...
										<div class="card-title">
											...
											<div class="d-flex align-items-center position-relative my-1">
												...
												<input type="text" data-kt-table="clients_table" data-kt-filter="search"
													class="form-control form-control-solid w-250px ps-12"
													placeholder="{{ __('general.search_keyword') }}" />
											</div>
											<!--end::Search-->
											<!--begin::Export buttons-->
											<div id="clients_table_export" class="d-none"></div>
											<!--end::Export buttons-->
										</div>
										<!--end::Card title-->
										<!--begin::Card toolbar-->
										<div class="card-toolbar flex-row-fluid justify-content-end gap-5">
											...
											<div class="w-150px">
												...
												<select class="form-select form-select-solid" data-control="select2"
													...
													data-kt-table="clients_table" data-kt-filter="status">
													<option></option>
    								        ...
    								        <!--begin::Menu-->
    								        <div id="clients_table_export_menu"
										...
										<table class="table table-hover align-middle table-row-dashed fs-6 kt-datatable gy-3"
											...
											<thead>
												<tr class=" text-gray-500 fw-bold fs-7 text-uppercase gs-0">
													...
													<th class="text-start min-w-100px">{{ __('general.name') }}</th>
													<th class="text-start min-w-100px">{{ __('general.currency') }}</th>
													<th class="text-start min-w-100px">{{ __('general.status') }}</th>
													<th class="text-start min-w-100px">{{ __('general.phone') }}</th>
													<th class="text-start min-w-100px">{{ __('general.email') }}</th>
											...
											<tbody class="fw-semibold text-gray-800">
												...
												<tr>
													...
													<td class="text-start">
														...
														<a href="{{route('clients.show' , $client->id)}}" class="text-primary-900 text-hover-primary">
														@endhasAccess
														{{$client->name}}</a>
													</td>
													<td class="text-start">{{$client->currency}}</td>
													<td class="text-start"><div class="badge {{ $client->status === 'active' ? 'badge-light-success' : 'badge-light-danger' }}">{{ __('general.' . $client->status) }}</div></td>
													...
													<td class="text-end">
														...
														<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
															...
															@if($client->status === 'active')
																<!--begin::Menu item-->
																<div class="menu-item px-3">
																	<a href="{{route('clients.disable', $client->id)}}"
																		class="menu-link px-3">{{ __('general.disable') }}</a>
																</div>
																<!--end::Menu item-->
															@else
																<!--begin::Menu item-->
																<div class="menu-item px-3">
																	<a href="{{route('clients.activate', $client->id)}}"
																		class="menu-link px-3">{{ __('general.activate') }}</a>
																</div>
																<!--end::Menu item-->
															@endif
															<!--begin::Menu item-->
															...
															<div class="menu-item px-3">
																<form id="delete-form-{{ $client->id }}" action="{{ route('clients.destroy', $client->id) }}" method="POST">
																	...
																	@method('DELETE')
																	<div class="menu-item">
																		<button type="button" onclick="showConfirmation('{{ addslashes($client->name) }}', '{{ $client->id }}');" class="dropdown-item menu-link fw-bold text-danger">{{ __('general.delete') }}</button>
																	</div>
																</form>
															</div>
															<!--end::Menu item-->
