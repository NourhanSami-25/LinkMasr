<div class="tab-pane fade" id="kt_customer_view_invoices" role="tabpanel">
                ...
                <div class="d-flex flex-wrap">
                    ...
                    <div class="border border-dashed border-gray-300 w-200px rounded my-3 p-4 me-6">                    
                        <span class="fs-1 fw-bold text-gray-800 lh-1">
                            <span>{{ number_format($invoicesTotals['paid'], 2) }} {{ $companyProfile->currency ?? 'EGP' }}</span>
                    ...
                    <div class="border border-dashed border-gray-300 w-200px rounded my-3 p-4 me-6">   
                        <span class="fs-1 fw-bold text-gray-800 lh-1">
                            <span>{{ number_format($invoicesTotals['unpaid'], 2) }} {{ $companyProfile->currency ?? 'EGP' }}</span>
                    ...
                    <div class="border border-dashed border-gray-300 w-200px rounded my-3 p-4 me-6">   
                        <span class="fs-1 fw-bold text-gray-800 lh-1">
                            <span>{{ number_format($invoicesTotals['partially_paid'], 2) }} {{ $companyProfile->currency ?? 'EGP' }}</span>
                    ...
                    <div class="border border-dashed border-gray-300 w-200px rounded my-3 p-4 me-6">
                        <span class="fs-1 fw-bold text-gray-800 lh-1">
                            <span>{{ number_format($invoicesTotals['draft'], 2) }} {{ $companyProfile->currency ?? 'EGP' }}</span>
    ...
    <div class="card pt-4 mb-6 mb-xl-9">
        ...
        <div class="card-header border-0">
            ...
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                ...
                <a href="{{route('invoices.create', ['client_id' => $client->id])}}" class="btn btn-flex btn-outline btn-primary h-40px fs-7 fw-bold" target="_blank">{{ __('general.create_new') }}</a>
				@endhasAccess
                @hasAccess('finance','view')
                <a href="{{route('invoices.index' )}}" class="btn btn-flex btn-outline btn-primary h-40px fs-7 fw-bold" target="_blank">{{ __('general.view_all') }}</a>
                @endhasAccess
            </div>
            <!--end::Card toolbar-->
        </div>
            ...
            <table class="table align-middle table-row-dashed kt-datatable gy-5"
                ...
                <tbody class="fs-6 fw-semibold text-gray-800">
                    ...
                    <tr>
                        ...
                        <td class="text-start">{{ __('general.' .$invoice->status) }}</td>
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                ...
                                <div class="menu-item px-3">
                                    <form id="delete-form-{{ $invoice->id }}" action="{{ route('invoices.destroy', $invoice->id) }}" method="POST">
                                        ...
                                        @method('DELETE')
                                        <div class="menu-item">
                                            <button type="button" onclick="showConfirmation('{{ addslashes($invoice->number) }}', '{{ $invoice->id }}');" class="dropdown-item menu-link fw-bold text-danger">{{ __('general.delete') }}</button>
                                        </div>
                                    </form>
                                </div>
                                <!--end::Menu item-->
