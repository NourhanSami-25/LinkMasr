...
<head>
	...
	<title>Link Masr | Create Task</title>
...
<body id="kt_app_body" data-kt-app-header-fixed="true" data-kt-app-header-fixed-mobile="true"
					...
					<div class="d-flex flex-column flex-column-fluid">
						...
						<div id="kt_app_content" class="app-content flex-column-fluid">
							...
							<div id="kt_app_content_container" class="app-container container-fluid">
								<!--begin::Form-->
								<form action="{{route('tasks.store')}}" method="POST" class="form d-flex flex-column flex-lg-row"
													...
													<div class="card card-flush py-4">
														...
														<div class="card-body pt-0">
															...
															<div class="d-flex flex-wrap gap-5 mb-5">
																...
																<div class="fv-row w-100 flex-md-root">
																	...
																	<select class="form-select mb-2" name="type" data-label="{{ __('general.type') }}"
																		...
																		<option></option>
																		<option value=""></option>
																		<option value="private" selected="selected">{{ __('general.private') }}</option>
															...
															<div class="d-flex flex-wrap gap-5 mb-5">
																...
																<!--begin::Input group - BOQ -->
																<div class="fv-row w-100 flex-md-root secondary-selects select-boq" style="display: none;">
																	<!--begin::Label-->
																	<label class="form-label">{{ __('general.boq_item') ?? 'BOQ Item' }} <small class="text-muted">({{ __('general.optional') ?? 'Optional' }})</small></label>
																	<!--end::Label-->
																	<!--begin::Select2-->
																	<select class="form-select mb-2" name="boq_id" data-label="{{ __('general.boq_item') ?? 'BOQ Item' }}"
																	    data-control="select2"
																	    data-placeholder="{{ __('general.select_boq_item') ?? 'Select BOQ Item' }}">
																	    <option value="">{{ __('general.none') ?? 'None' }}</option>
																	</select>
																	<!--end::Select2-->
																	<small class="text-muted">{{ __('general.boq_link_help') ?? 'Link to BOQ item for EVM tracking' }}</small>
																</div>
																<!--end::Input group-->
																...
																<div class="fv-row w-100 flex-md-root">
																	...
																	<select class="form-select mb-2" name="is_billed" data-label="{{ __('general.is_billed') }}"
																		...
																		<option></option>
																		<option value="1" selected="selected">{{ __('general.yes') }}</option>
																...
																<div class="fv-row w-100 flex-md-root">
																	...
																	<select class="form-select mb-2" name="priority" data-label="{{ __('general.priority') }}"
																		...
																		<option></option>
																		<option value="low" selected="selected">{{ __('general.low') }}</option>
															...
															<div class="mb-5 mt-5">
																...
																<textarea class="form-control mb-2"
																	value="{{ old('description') }}" name="description" data-label="{{ __('general.description') }}" data-maxlength="1024"></textarea>
																<!--end::Editor-->
																<!--begin::Description-->
																<div class="text-muted fs-7">{{ __('general.optional_message') }}</div>
																<!--end::Description-->
														...
														<div class="card-body pt-0">
															...
															<div class="d-flex flex-wrap gap-5 mb-5">
																...
																<div class="fv-row w-100 flex-md-root">
																	...
																	<!--end::Label-->
																	<!--begin::Input-->
																	<input class="flatpickr-date form-control mb-2" name="date" data-label="{{ __('general.start_date') }}" value="{{ date('Y-m-d H:i') }}"  data-type="dateTime" data-required="true"/>
																	<!--end::Input-->
																</div>
																<!--end::Input group-->
																<!--begin::Input group-->
																<div class="fv-row w-100 flex-md-root">
																	<!--begin::Label-->
																	<label class="required form-label">{{ __('general.dead_line') }}</label>
																	<!--end::Label-->
																	<!--begin::Input-->
																	<input class="flatpickr-date form-control mb-2" name="due_date" data-label="{{ __('general.dead_line') }}" data-type="dateTime" data-required="true"/>
																	<!--end::Input-->
																</div>
																<!--end::Input group-->
															</div>
	...
	<!-- BOQ Integration Script -->
	<script>
		document.addEventListener('DOMContentLoaded', function() {
			const projectSelect = document.querySelector('select[name="project_id"]');
			const boqSelect = document.querySelector('select[name="boq_id"]');
			const boqContainer = document.querySelector('.select-boq');
			
			if (projectSelect && boqSelect) {
				projectSelect.addEventListener('change', function() {
					const projectId = this.value;
					if (projectId) {
						// Show BOQ container
						boqContainer.style.display = 'block';
						
						// Fetch BOQ items for this project
						fetch(`/api/projects/${projectId}/boq-items`)
							.then(response => response.json())
							.then(data => {
								boqSelect.innerHTML = '<option value="">{{ __("general.none") ?? "None" }}</option>';
								data.forEach(item => {
									const option = document.createElement('option');
									option.value = item.id;
									option.textContent = item.code + ' - ' + (item.item_description || item.item_name || 'Item');
									boqSelect.appendChild(option);
								});
							})
							.catch(err => {
								console.log('No BOQ items or error:', err);
								boqContainer.style.display = 'none';
							});
					} else {
						boqContainer.style.display = 'none';
					}
				...
				// Trigger change if project is pre-selected
