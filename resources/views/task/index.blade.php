							...
							<div id="kt_app_toolbar_container"
								...
								<div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
									...
									<div class="d-flex align-items-center gap-2 gap-lg-3">
										...
										<a href="{{route('tasks.create')}}" class="btn btn-flex btn-primary h-40px fs-7 fw-bold">{{ __('general.create_new') }}</a>
										@endhasAccess
    								...
    								<div class="card-header align-items-center py-5 gap-2 gap-md-5">
    								    ...
    								    <div class="card-title">
    								        ...
    								        <div class="d-flex align-items-center position-relative my-1">
    								            ...
    								            <input type="text" data-kt-table="tasks_table" data-kt-filter="search"
    								                class="form-control form-control-solid w-250px ps-12"
    								                placeholder="{{ __('general.search_keyword') }}" />
    								        </div>
    								        <!--end::Search-->
    								        <!--begin::Export buttons-->
    								        <div id="tasks_table_export" class="d-none"></div>
    								        <!--end::Export buttons-->
    								    </div>
    								    <!--end::Card title-->
    								    <!--begin::Card toolbar-->
    								    <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
    								        ...
    								        <div class="w-150px">
    								            ...
    								            <select class="form-select form-select-solid" data-control="select2"
    								                ...
    								                data-kt-table="tasks_table" data-kt-filter="status">
    								                <option></option>
    								        ...
    								        <!--begin::Menu-->
    								        <div id="tasks_table_export_menu"
    								    ...
    								    <table class="table table-hover align-middle table-row-dashed fs-6 kt-datatable gy-5"
    								        ...
    								        <thead>
    								            <tr class="text-gray-500 fw-bold fs-7 text-uppercase gs-0">
    								                ...
    								                <th class="text-start min-w-100px">{{ __('general.asignees') }}</th>
    								                <th class="text-start min-w-100px">{{ __('general.followers') }}</th>
											...
											<tbody class="fw-semibold text-gray-800">
												...
												<tr>
													...
													<td class="text-start">{{__('general.' . $task->related)}} </td>
													<td class="text-start">
														<div class="badge 
													...
													<td class="text-start">
														<ul style="list-style-type: none; padding: 0; display: flex; gap: 10px;">
														    @foreach ($task->assignees_names as $assignee)
																@php
													...
													<td class="text-start">
														<ul style="list-style-type: none; padding: 0; display: flex; gap: 10px;">
														    @foreach ($task->followers_names as $follower)
																@php
        														    // Extract first letter of each word
        														    $initials = collect(explode(' ', $follower))->map(fn($word) => Str::substr($word, 0, 1))->join('');
        														@endphp
														        <li>
																	<div class="symbol-group symbol-hover mb-3" style="margin-left: -10px">
																		<div class="symbol symbol-35px symbol-circle">
													...
													<td class="text-end">
														...
														<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
															...
															<!--begin::Menu item-->
															<div class="menu-item px-3">
															    <a href="{{ route('tasks.change-status', ['task' => $task->id, 'status' => 'in_progress']) }}"
															        class="menu-link px-3 {{ $task->status === 'in_progress' ? 'disabled' : '' }}">{{ __('general.mark_as_in_progress') }}</a>
															</div>
															<div class="menu-item px-3">
															    <a href="{{ route('tasks.change-status', ['task' => $task->id, 'status' => 'on_hold']) }}"
															        class="menu-link px-3 {{ $task->status === 'on_hold' ? 'disabled' : '' }}">{{ __('general.mark_as_on_hold') }}</a>
															</div>
															<div class="menu-item px-3">
															    <a href="{{ route('tasks.change-status', ['task' => $task->id, 'status' => 'not_started']) }}"
															        class="menu-link px-3 {{ $task->status === 'not_started' ? 'disabled' : '' }}">{{ __('general.mark_as_not_started') }}</a>
															</div>
															<div class="menu-item px-3">
															    <a href="{{ route('tasks.change-status', ['task' => $task->id, 'status' => 'completed']) }}"
															        class="menu-link px-3 {{ $task->status === 'completed' ? 'disabled' : '' }}">{{ __('general.mark_as_completed') }}</a>
															</div>
															<!--end::Menu item-->
															<!--begin::Menu item-->
															<div class="menu-item px-3">
