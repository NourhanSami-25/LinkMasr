								...
								<div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
									...
									<div class="page-title d-flex flex-column justify-content-center gap-1 me-3">
										...
										<ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0">
											...
											<!--begin::Item-->
											<li class="breadcrumb-item text-muted">
												<a href="{{ route('roles.index')}}" class="text-muted text-hover-primary">{{ __('general.roles') }}</a>
											</li>
											<!--end::Item-->
											<!--begin::Item-->
											<li class="breadcrumb-item">
											...
											<!--end::Item-->
											<!--begin::Item-->
											<li class="breadcrumb-item text-muted">{{ __('general.users_roles') }}</li>
											<!--end::Item-->
										</ul>
										<!--end::Breadcrumb-->
									</div>
									...
									<div class="card-header align-items-center py-5 gap-2 gap-md-5">
										...
										<div class="card-title">
											...
											<div class="d-flex align-items-center position-relative my-1">
												...
												<input type="text" data-kt-table="users_table" data-kt-filter="search"
													class="form-control form-control-solid w-250px ps-12"
													placeholder="{{ __('general.search_keyword') }}" />
											</div>
											<!--end::Search-->
											<!--begin::Export buttons-->
											<div id="users_table_export" class="d-none"></div>
											<!--end::Export buttons-->
										</div>
										<!--end::Card title-->
										<!--begin::Card toolbar-->
										<div class="card-toolbar flex-row-fluid justify-content-end gap-5">
											...
											<div class="w-150px">
												...
												<select class="form-select form-select-solid" data-control="select2"
													data-hide-search="true" data-placeholder="{{ __('general.position') }}"
													data-kt-table="users_table" data-kt-filter="status">
													<option></option>
													<option value="all">{{ __('general.all') }}</option>
													@foreach($positions as $position)
														<option value="{{$position->subject}}">{{$position->subject}}</option>		
													@endforeach																								
												</select>
												<!--end::Select2-->
											</div>
											...
											<!--begin::Menu-->
											<div id="users_table_export_menu"
										...
										<table class="table table-hover align-middle table-row-dashed fs-6 kt-datatable gy-5"
											...
											<thead>
												<tr class=" text-gray-500 fw-bold fs-7 text-uppercase gs-0">
													...
													<th class="text-start min-w-50px">{{ __('general.department') }}</th>
													<th class="text-start min-w-50px">{{ __('general.position') }}</th>
													<th class="text-start min-w-200px">{{ __('general.is_admin') }}</th>
													...
													{{-- <td class="text-start">
                                                        @foreach($user->roles as $role)
                                                            <span class="badge bg-primary text-light fs-6">
                                                                {{ $role->subject }} ({{ ucfirst($role->pivot->access_level) }})
                                                            </span>
                                                        @endforeach
                                                    </td> --}}
														<td class="text-end">
														...
														<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
															...
															<!--begin::Menu item-->
															<div class="menu-item px-3">
																<a href="{{route('users_roles_edit', $user->id)}}"
