...
<head>
	...
	<title>Link Masr | ExchangeRates</title>
								...
								<div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
									...
									<div class="page-title d-flex flex-column justify-content-center gap-1 me-3">
										...
										<h1
											...
											{{ __('general.all_exchange_ratess') }} </h1>
										<!--end::Title-->
										<!--begin::Breadcrumb-->
										<ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0">
											...
											<!--end::Item-->
											<!--begin::Item-->
											<li class="breadcrumb-item text-muted">{{ __('general.setting') }}</li>
											<!--end::Item-->
											<!--begin::Item-->
											<li class="breadcrumb-item">
												<span class="bullet bg-gray-500 w-5px h-2px"></span>
											...
											<!--begin::Item-->
											<li class="breadcrumb-item text-muted">
												<a href="{{ route('exchangeRates.index')}}" class="text-muted text-hover-primary">{{ __('general.exchange_rates') }}</a>
											</li>
											<!--end::Item-->
										</ul>
										<!--end::Breadcrumb-->
								...
								<div class="card card-flush">
									...
									<div class="card-header align-items-center py-5 gap-2 gap-md-5">
										...
										<div class="card-title">
											...
											<div class="d-flex align-items-center position-relative my-1">
												...
												<input type="text" data-kt-table="exchangeRates_table" data-kt-filter="search"
													class="form-control form-control-solid w-250px ps-12"
													placeholder="{{ __('general.search_keyword') }}" />
											</div>
											<!--end::Search-->
											<!--begin::Export buttons-->
											<div id="exchangeRates_table_export" class="d-none"></div>
											<!--end::Export buttons-->
										</div>
										<!--end::Card title-->
										<!--begin::Card toolbar-->
										<div class="card-toolbar flex-row-fluid justify-content-end gap-5">
											...
											<!--begin::Menu-->
											<div id="exchangeRates_table_export_menu"
									...
									<div class="card-body pt-0">
										...
										<table class="table table-hover align-middle table-row-dashed fs-6 kt-datatable gy-3"
											id="exchangeRates_table">
											<thead>
												<tr class=" text-gray-500 fw-bold fs-7 text-uppercase gs-0">
													<th class="text-start min-w-50px">#</th>
													<th class="text-start min-w-50px">{{ __('general.currency') }}</th>
													<th class="text-start min-w-150px">{{ __('general.exchange_rate') }}</th>
													<th class="text-start min-w-100px">{{ __('general.created_at') }}</th>
													<th class="text-start min-w-100px">{{ __('general.updated_at') }}</th>
											...
											<tbody class="fw-semibold text-gray-800">
												...
												<tr>
													<td class="text-start">{{$exchangeRate->id}}</td>
													<td class="text-start">{{ __("general." . strtolower($exchangeRate->currency)) }}</td>
													<td class="text-start">{{$exchangeRate->rate}}</td>
													<td class="text-start">{{$exchangeRate->created_at}}</td>
													<td class="text-start">{{$exchangeRate->updated_at}}</td>
													<td class="text-end">
														<a href="#"
														...
														<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
															...
															<div class="menu-item px-2">
																<form id="delete-form-{{ $exchangeRate->id }}" action="{{ route('exchangeRates.destroy', $exchangeRate->id) }}" method="POST">
																	@csrf
																	@method('DELETE')
																	<div class="menu-item">
																		<button type="button" onclick="showConfirmation('{{ addslashes($exchangeRate->subject) }}', '{{ $exchangeRate->id }}');" class="dropdown-item menu-link fw-bold text-danger">{{ __('general.delete') }}</button>
																	</div>
																</form>
															</div>
															<!--end::Menu item-->
