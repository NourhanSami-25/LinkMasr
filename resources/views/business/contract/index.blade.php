...
<head>
	...
	<title>Link Masr | Contracts</title>
...
<body id="kt_app_body" data-kt-app-header-fixed="true" data-kt-app-header-fixed-mobile="true"
									...
									<div class="card-header align-items-center py-5 gap-2 gap-md-5">
										...
										<div class="card-title">
											...
											<div class="d-flex align-items-center position-relative my-1">
												...
												<input type="text" data-kt-table="contracts_table" data-kt-filter="search"
													class="form-control form-control-solid w-250px ps-12"
													placeholder="{{ __('general.search_keyword') }}" />
											</div>
											<!--end::Search-->
											<!--begin::Export buttons-->
											<div id="contracts_table_export" class="d-none"></div>
											<!--end::Export buttons-->
										</div>
										<!--end::Card title-->
										<!--begin::Card toolbar-->
										<div class="card-toolbar flex-row-fluid justify-content-end gap-5">
											...
											<div class="w-150px">
												...
												<select class="form-select form-select-solid" data-control="select2"
													...
													data-kt-table="contracts_table" data-kt-filter="status">
													<option></option>
													...
													<option value="{{ __('general.about_to_expire') }}">{{ __('general.about_to_expire') }}</option>													
													<option value="{{ __('general.recently_added') }}">{{ __('general.recently_added') }}</option>													
													<option value="{{ __('general.accepted') }}">{{ __('general.accepted') }}</option>													
													<option value="{{ __('general.trash') }}">{{ __('general.trash') }}</option>													
												</select>
												<!--end::Select2-->
											</div>
											...
											<!--begin::Menu-->
											<div id="contracts_table_export_menu"
											...
											<tbody class="fw-semibold text-gray-800">
												...
												<tr>
													...
													<td class="text-start">
														...
														<a href="{{route('contracts.show' , $contract->id)}}" class="text-primary-900 text-hover-primary">
															@endhasAccess
														{{$contract->subject}}</a>
													</td>
													<td class="text-start">{{$contract->client_name}}</td>
													<td class="text-start">
														<div class="badge 
															{{ $contract->status === 'active' ? 'badge-light-success' : '' }}
															{{ $contract->status === 'expired' ? 'badge-light-danger' : '' }}
															{{ $contract->status === 'about_to_expire' ? 'badge-light-warning' : '' }}
															{{ $contract->status === 'recently_added' ? 'badge-light-primary' : '' }}
															{{ $contract->status === 'accepted' ? 'badge-light-info' : '' }}
															{{ $contract->status === 'trash' ? 'badge-light-secondary' : '' }}">
	...
	<script>
    	document.addEventListener('DOMContentLoaded', function () {
    	    const table = document.querySelector('#contracts_table');
    	    if (table) {
    	        $(table).DataTable({
