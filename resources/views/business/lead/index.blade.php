...
<body id="kt_app_body" data-kt-app-header-fixed="true" data-kt-app-header-fixed-mobile="true"
									...
									<div class="card-header align-items-center py-5 gap-2 gap-md-5">
										...
										<div class="card-title">
											...
											<div class="d-flex align-items-center position-relative my-1">
												...
												<input type="text" data-kt-table="leads_table" data-kt-filter="search"
													class="form-control form-control-solid w-250px ps-12"
													placeholder="{{ __('general.search_keyword') }}" />
											</div>
											<!--end::Search-->
											<!--begin::Export buttons-->
											<div id="leads_table_export" class="d-none"></div>
											<!--end::Export buttons-->
										</div>
										<!--end::Card title-->
										<!--begin::Card toolbar-->
										<div class="card-toolbar flex-row-fluid justify-content-end gap-5">
											...
											<div class="w-150px">
												...
												<select class="form-select form-select-solid" data-control="select2"
													...
													data-kt-table="leads_table" data-kt-filter="status">
													<option></option>
													<option value="all">{{ __('general.all') }}</option>
													<option value="{{ __('general.in_progress') }}">{{ __('general.in_progress') }}</option>													
													<option value="{{ __('general.contracted') }}">{{ __('general.contracted') }}</option>													
													<option value="{{ __('general.canceld') }}">{{ __('general.canceld') }}</option>																									
												</select>
												<!--end::Select2-->
											</div>
											<!--end::Filter-->
											...
											<!--begin::Menu-->
											<div id="leads_table_export_menu"
										...
										<table class="table table-hover align-middle table-row-dashed fs-6 kt-datatable gy-3"
											...
											<thead>
												<tr class=" text-gray-500 fw-bold fs-7 text-uppercase gs-0">
													...
													<th class="text-start min-w-100px">{{ __('general.source') }}</th>
													<th class="text-start min-w-100px">{{ __('general.lead_value') }}</th>
													<th class="text-start min-w-100px">{{ __('general.created_since') }}</th>
											...
											<tbody class="fw-semibold text-gray-800">
												...
												<tr>
													<td class="text-start">{{$lead->id}}</td>
													<td class="text-start">
														@hasAccess('lead','details')
														<a href="{{route('leads.show' , $lead->id)}}" class="text-primary-900 text-hover-primary">
														@endhasAccess
														{{$lead->subject}}</a></td>
													<td class="text-start">{{$lead->client_name}}</td>
													<td class="text-start">
														<div class="badge 
															{{ $lead->status === 'in_progress' ? 'badge-light-warning' : '' }}
															{{ $lead->status === 'contracted' ? 'badge-light-success' : '' }}
															{{ $lead->status === 'canceld' ? 'badge-light-danger' : '' }}">
													...
													<td class="text-end">
														...
														<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
															...
															<!--begin::Menu item-->
															<div class="menu-item px-3">
																<form id="delete-form-{{ $lead->id }}" action="{{ route('leads.destroy', $lead->id) }}" method="POST">
																	@csrf
																	@method('DELETE')
																	<div class="menu-item">
																		<button type="button" onclick="showConfirmation('{{ addslashes($lead->subject) }}', '{{ $lead->id }}');" class="dropdown-item menu-link fw-bold text-danger">{{ __('general.delete') }}</button>
																	</div>
																</form>
															</div>
															<!--end::Menu item-->
	...
	<script>
    	document.addEventListener('DOMContentLoaded', function () {
    	    const table = document.querySelector('#leads_table');
    	    if (table) {
    	        $(table).DataTable({
