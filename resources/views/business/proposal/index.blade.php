...
<head>
	...
	<title>Link Masr | Proposals</title>
...
<body id="kt_app_body" data-kt-app-header-fixed="true" data-kt-app-header-fixed-mobile="true"
									...
									<div class="card-header align-items-center py-5 gap-2 gap-md-5">
										...
										<div class="card-title">
											...
											<div class="d-flex align-items-center position-relative my-1">
												...
												<input type="text" data-kt-table="proposals_table" data-kt-filter="search"
													class="form-control form-control-solid w-250px ps-12"
													placeholder="{{ __('general.search_keyword') }}" />
											</div>
											<!--end::Search-->
											<!--begin::Export buttons-->
											<div id="proposals_table_export" class="d-none"></div>
											<!--end::Export buttons-->
										</div>
										<!--end::Card title-->
										<!--begin::Card toolbar-->
										<div class="card-toolbar flex-row-fluid justify-content-end gap-5">
											...
											<div class="w-150px">
												...
												<select class="form-select form-select-solid" data-control="select2"
													...
													data-kt-table="proposals_table" data-kt-filter="status">
													<option></option>
													<option value="all">{{ __('general.all') }}</option>									
													<option value="{{ __('general.sent') }}">{{ __('general.sent') }}</option>													
													<option value="{{ __('general.open') }}">{{ __('general.open') }}</option>													
													<option value="{{ __('general.revise') }}">{{ __('general.revise') }}</option>													
											...
											<!--begin::Menu-->
											<div id="proposals_table_export_menu"
										...
										<table class="table table-hover align-middle table-row-dashed fs-6 kt-datatable gy-3"
											...
											<thead>
												<tr class=" text-gray-500 fw-bold fs-7 text-uppercase gs-0">
													...
													<th class="text-start min-w-100px">{{ __('general.number') }}</th>
											...
											<tbody class="fw-semibold text-gray-800">
												...
												<tr>
													...
													<td class="text-start">
														...
														<a href="{{route('proposals.show' , $proposal->id)}}" class="text-primary-900 text-hover-primary">
														@endhasAccess
														{{$proposal->subject}}</a></td>
													<td class="text-start">{{$proposal->client_name}}</td>
													<td class="text-start">
														<div class="badge 
															{{ $proposal->status === 'sent' ? 'badge-light-primary' : '' }}
															{{ $proposal->status === 'open' ? 'badge-light-info' : '' }}
															{{ $proposal->status === 'revise' ? 'badge-light-warning' : '' }}
															{{ $proposal->status === 'accepted' ? 'badge-light-success' : '' }}
															{{ $proposal->status === 'declined' ? 'badge-light-danger' : '' }}
															{{ $proposal->status === 'draft' ? 'badge-light-secondary' : '' }}">
													...
													<td class="text-start">{{date('Y-m-d', strtotime($proposal->due_date))}}</td>
													<td class="text-start">{{__getUserNameById ($proposal->created_by)}}</td>
													<td class="text-end">
														<a href="#"
														...
														<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
															...
															<div class="menu-item px-3">
																<a href="{{route('proposals.show', $proposal->id)}}"
																	class="menu-link px-3">{{ __('general.print&download') }}</a>
															</div>
															<!--end::Menu item-->
															@endhasAccess
	...
	<script>
    	document.addEventListener('DOMContentLoaded', function () {
    	    const table = document.querySelector('#proposals_table');
    	    if (table) {
    	        $(table).DataTable({
